...
def run_WA_MCA(supercell_obj,numb_passes,num_sub_passes,temp,temp_inc,Cluster_rules,J_rules,Js,species_flip,do_figs=True):
    T = temp
    Kb = .000086173324
    inc_down = 0
    inc_up = 0
    ...
    for passes in range(numb_passes):
        ...
        plt.subplot(312)
        plt.xlabel("Sweeps", fontsize=10)
        plt.ylabel("abs(Average Mag)", fontsize=10)
        plt.plot(X_axis,np.abs(total_spin),lw=3,marker='o',color=c)###########
        plt.figure(4)
        plt.subplot(411)
        plt.xlabel("Sweeps", fontsize=10)
